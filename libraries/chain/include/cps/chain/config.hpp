...
namespace cpsio { namespace config {
using types::uint16;
using types::uint32;
using types::uint64;
using types::uint128;
...
const static char key_prefix[] = "CPS";

const static account_name cps_contract_name = N(cps);
...
const static share_type initial_token_supply = asset::from_string("1000000000.00000000 CPS").amount;
...
const static share_type default_min_cps_balance = asset(100).amount;
...
} } // namespace cpsio::config

template<typename Number>
Number cps_percent(Number value, int percentage) {
   return value * percentage / cpsio::config::percent100;
}
